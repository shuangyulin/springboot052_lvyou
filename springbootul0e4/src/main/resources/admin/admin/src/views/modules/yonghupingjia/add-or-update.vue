<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="观光日期" prop="guanguangriqi">
          <el-input v-model="ruleForm.guanguangriqi" 
              placeholder="观光日期" clearable  :readonly="ro.guanguangriqi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="观光日期" prop="guanguangriqi">
              <el-input v-model="ruleForm.guanguangriqi" 
                placeholder="观光日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jingdianmingcheng = true;
          ...
          if(o=='guanguangriqi'){
            this.ruleForm.guanguangriqi = obj[o];
	    this.ro.guanguangriqi = true;
            continue;
          }
          if(o=='yonghuming'){
	...
	this.ruleForm.pingjianeirong = this.ruleForm.pingjianeirong.replace(reg,'../../../springbootul0e4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yonghupingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yonghupingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
